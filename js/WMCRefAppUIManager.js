...
class WMCRefAppUIManager {
    ...
    populateSourceListItemsFromConfig(sourceListConfig) {
        ...
        let populateSourceListItems = function (sourceList) {
            ...
            if (_this._uiDOMElement.sourceList.list) {
                ...
                for (var i = 0, envs = Object.keys(_this._sourceList_env), ii = envs.length; i < ii; i++) {
                    ...
                    const sourceEnvTabHeader = new DOMParser().parseFromString(
                        `<div class="card-header">
                            <a class="collapsed card-link" data-toggle="collapse" href="#${envLocator}-env-card">
                                ${env}
                            </a>
                        </div>`,
                    ...
                    if (sourceEnvTabHeader) {
                        _this._uiDOMElement.sourceList.list.appendChild(sourceEnvTabHeader);
                    }

                    const sourceEnvTabBody = new DOMParser().parseFromString(
                        `<div id="${envLocator}-env-card" class="collapse${counter === 0 ? ' show' : ''}" data-parent="#source-list-accordion">
                            <div class="card-body">
                    ...
                    // append the source list item to the source list
                    if (sourceEnvTabBody) {
                        _this._uiDOMElement.sourceList.list.appendChild(sourceEnvTabBody);
                    }
                        ...
                        const sourceListItem = new DOMParser().parseFromString(
                            `<a id="${env}-source-list-item-${index}" role="button" class="list-group-item list-group-item-action flex-column align-items-start source-list-item">
                                <div class="d-flex w-100 justify-content-between">
                                    ...
                                    <span class="badge ${mode.toLowerCase() === 'vod' ? 'badge-warning' : 'badge-success'}">${mode}</span>
                                </div>
                ...
                $(".source-list-item").click(function () {
                    // Select all source items
                    let sourceItems = $(".source-list-item");
                    ...
                    onSourceListItemSelected(this.id);
                });
            }
        };
                            ...
                            <div id="${env}-source-list-item-${index}" class="col-lg-6 col-xl-3 mb-5 source-list-item">
                                <div class="card h-100 text-center">
                                    <div class="card-header text-right">
                                        <div class="badge ${mode.toLowerCase() === 'vod' ? 'badge-danger' : 'badge-success'}">
                                            ${mode}
                                        </div>
        ...
        let populateSourceListItems = function (sourceList) {
            ...
            if (_this._uiDOMElement.sourceList.list) {
                ...
                for (var i = 0, envs = Object.keys(_this._sourceList_env), ii = envs.length; i < ii; i++) {
                    ...
                    const sourceEnvTabHeader = new DOMParser().parseFromString(
                        `<div class="card-header">
                            <a class="collapsed card-link" data-toggle="collapse"href="#${env}-env-card">
                                ${env}
                            </a>
                        </div>`,
                        "text/html").body.firstChild;
                    ...
                    const sourceEnvTabBody = new DOMParser().parseFromString(
                        `<div id="${env}-env-card" class="collapse${i === 0 ? ' show' : ''}" data-parent="#source-list-accordion">
                            <div class="card-body">
                                <div class="list-group source-list" id="${env}-source-list">
                                    <!-- JavaScript will populate the source list items if a valid source list config is fed to it. -->
                                </div>
     ...
     * @param {*} duration
     */
    updateAdMarkersValue(markers, duration) {
    ...
    constructor() {
        ...
        if (window && window.location && window.location.pathname) {
            ...
            if (window.location.pathname.split("/").pop() === "" || window.location.pathname.split("/").pop() === "index.html" || window.location.pathname.split("/").pop() === "index_d.html") {
