...
function onPlayerEvent(eventObj) {
    ...
    switch (eventType) {
        ...
        case AmcEvents.AMC_EVENT_PLAY_READY:
            ...
            // start preloading if contentAdvisory check is enabled for VoD, else start playback
            if (wmcPlayerConfig.playbackMode === "VOD" && wmcPlayerConfig.contentAdvisoryEnabled) {
                // start preloading the VoD asset
                wmcPlayer.preload();
            } else {
        ...
        /* This event is received after a preload was requested and when preloading completes */
        case AmcEvents.AMC_EVENT_PRELOAD_COMPLETE:
        ...
        case AmcEvents.AMC_EVENT_VIDEO_POSITION_CHANGED:
            ...
            if (isExternalSource && wmcPlayerConfig.playbackMode === "LIVE") {
...
function registerEventListeners() {
    ...
    wmcPlayer.addEventListener(AmcEvents.AMC_EVENT_PRELOAD_COMPLETE, onPlayerEvent);
...
function unregisterEventListeners() {
    ...
    wmcPlayer.removeEventListener(AmcEvents.AMC_EVENT_PRELOAD_COMPLETE, onPlayerEvent);
...
function setPlayerConfigParams(configParams) {
    ...
    if (configParams.hasOwnProperty("analyticsEnabled") && configParams.analyticsEnabled) {
...
function createPlayer(videoElement, subtitleContainer = null, configParams = {}) {
    ...
    // TODO: Passing player key and analytics key
    // wmcPlayer.setPlayerKey("xxxxxx-xxxxxx-xxxxxxx");
    // wmcPlayer.setAnalyticsConfig("xxxxxx-xxxxxx-xxxxxxx", "dummy@abc.com", "LIVE$4346", "Dummy Test Channel");
...
function onPlayPause() {
    ...
    if (wmcPlayer) {
        if (wmcPlayerState === WMCRefAppPlayerState.Idle) {
            ...
            if (!isExternalSource && wmcPlayerConfig.playbackMode === "LIVE" && !wmcPlayer.timeshiftEnabled()) {
...
function onRestart() {
    ...
    if (wmcPlayer && wmcPlayerState !== WMCRefAppPlayerState.Idle) {
        ...
        const seekPosition = (wmcPlayerConfig.playbackMode === "LIVE" && wmcPlayer.timeshiftEnabled()) ? 1 : 0;
        if (isSeekableRange && wmcPlayerConfig.playbackMode === "LIVE") {
...
function onSkipAd() {
    if (wmcPlayer && wmcPlayerState !== WMCRefAppPlayerState.Idle && uiMgr._markers) {
        ...
        let programDuration = wmcPlayer.getVideoDuration();
        let seekTo = getActualSkipAdvertisementTime(currentPosition, programDuration);
 ...
 */
function getActualSkipAdvertisementTime(pos, dur) {
    pos = uiMgr.fixedTo(pos, 2);
    dur = uiMgr.fixedTo(dur, 2);
